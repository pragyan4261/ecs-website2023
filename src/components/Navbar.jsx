...
const Navbar = (props) => {
  const navs_left = [{ text: 'home', type: 'route', dest: props.home === 'active' ? '#' : '/' }, { text: 'events', type: 'route', dest: props.events === 'active' ? '#' : '/events' }, { text: 'feed', type: 'route', dest: props.feed === 'active' ? '#' : '/feed' }, { text: 'about us', type: 'route', dest: props.about === 'active' ? '#' : '/about' }];
    ...
    <div>
      ...
      <div className='invisible flex justify-around pl-7 pr-7 items-center h-0 md:visible md:h-[5rem]'>
        <div className='flex justify-between items-center w-[80%] z-4'>
          {navs_left.map((item) => <NavLink text={item.text} type={item.type} dest={item.dest} />)}
          <img src={Logo} alt="" className='h-0 w-0 md:h-10 md:w-24' />
          {navs_right.map((item) => <NavLink text={item.text} type={item.type} dest={item.dest} />)}
        </div>
      </div>
    </div>
